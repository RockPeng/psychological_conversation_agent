...
with st.sidebar:
    ...
    # 用户信息配置
    st.subheader("用户信息")
    age = st.number_input("年龄", min_value=0, max_value=120, value=st.session_state.user_info["age"] or 25)
    gender = st.radio("性别", ["男", "女", "其他"], index=0 if st.session_state.user_info["gender"] is None else ["男", "女", "其他"].index(st.session_state.user_info["gender"]))
